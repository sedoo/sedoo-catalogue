...
class multi_instru_dataset extends base_dataset
{
    ...
    public function display($project_name)
    {
        ...
        echo "<tr><td><strong>Period</strong></td><td colspan='3'>" . $this->period->period_name . "</td></tr>";
        echo "<tr><td><strong>Date begin</strong></td><td>" . $this->dats_date_begin . "</td>";
        echo "<td><strong>Date end</strong></td><td>" . (($this->dats_date_end_not_planned) ? 'not planned' : $this->dats_date_end) . "</td></tr>";

        echo "<tr><td><strong>Contact(s)</strong></td><td colspan='3'>";
        ...
        echo "<tr><td><strong>Site name</strong></td><td colspan='3'>" . $this->sites[0]->place_name . "</td></tr>";
        echo "<tr><td><strong>Plateform type</strong></td><td colspan='3'>" . $this->sites[0]->gcmd_plateform_keyword->gcmd_plat_name . "</td></tr>";
        if (isset($this->sites[0]->parent_place) && !empty($this->sites[0]->parent_place)) {
            echo "<tr><td><strong>Predefined site</strong></td><td colspan='3'>" . printPredefinedSite($this->sites[0]->parent_place) . "</td></tr>";
            echo "<tr><td><strong>Site type</strong></td><td colspan='3'>" . $this->sites[0]->parent_place->gcmd_plateform_keyword->gcmd_plat_name . "</td></tr>";
        }
        if ($this->dats_abstract) {
            echo "<tr><td><strong>Abstract</strong></td><td colspan='3'>" . tmp_format_abstract($this->dats_abstract) . "</td></tr>";
        }
        if ($this->dats_purpose) {
            echo "<tr><td><strong>Observing strategy</strong></td><td colspan='3'>" . tmp_format_abstract($this->dats_purpose) . "</td></tr>";
        }
      ...
      //Carte
        ...
        for ($i = 0, $size = count($this->dats_sensors); $i < $size; $i++) {
            ...
            if (isset($this->dats_sensors[$i]->sensor->gcmd_instrument_keyword)) {
                echo "<tr><td><strong>Instrument type</strong></td><td colspan='3'>" . $this->dats_sensors[$i]->sensor->gcmd_instrument_keyword->gcmd_sensor_name . "</td></tr>";
            }
            if (isset($this->dats_sensors[$i]->sensor->manufacturer)) {
                echo "<tr><td><strong>Manufacturer</strong></td><td colspan='3'>" . $this->dats_sensors[$i]->sensor->manufacturer->manufacturer_name;
            }
            ...
            echo "<tr><td><strong>Model</strong></td><td colspan='3'>" . $this->dats_sensors[$i]->sensor->sensor_model . "</td></tr>";
            if ($this->dats_sensors[$i]->sensor->sensor_url) {
                echo "<tr><td><strong>Reference</strong></td><td colspan='3'>" . $this->dats_sensors[$i]->sensor->sensor_url . "</td></tr>";
            }
            if ($this->dats_sensors[$i]->sensor->sensor_calibration) {
                echo "<tr><td><strong>Instrument features / Calibration</strong></td><td colspan='3'>" . $this->dats_sensors[$i]->sensor->sensor_calibration . "</td></tr>";
            }
            displayUtils::displaySensorResolution($this->dats_sensors[$i]);
            if (isset($this->dats_sensors[$i]->sensor->boundings)) {
                echo "<tr><td><strong>Longitude (°)</strong></td><td>" . $this->dats_sensors[$i]->sensor->boundings->west_bounding_coord . "</td><td><strong>Latitude (°)</strong></td><td>" . $this->dats_sensors[$i]->sensor->boundings->north_bounding_coord . "</td></tr>";
            }
            if ($this->dats_sensors[$i]->sensor->sensor_elevation || $this->dats_sensors[$i]->sensor->sensor_height) {
                echo "<tr><td><strong>Sensor altitude (m)</strong></td><td>" . $this->dats_sensors[$i]->sensor->sensor_elevation . "</td>";
                echo "<td><strong>Height above ground (m)</strong></td><td>" . $this->dats_sensors[$i]->sensor->sensor_height . "</td></tr>";
            }
            if ($this->dats_sensors[$i]->sensor->sensor_environment) {
                echo "<tr><td><strong>Instrument environment</strong></td><td colspan='3'>" . $this->dats_sensors[$i]->sensor->sensor_environment . "</td></tr>";
            }
