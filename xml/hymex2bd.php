...
function makeDatsBoundings($point, $rectangle, $disc, $line)
{
    ...
    } elseif (isset($rectangle) && !empty($rectangle)) {
        ...
        while (($file = readdir($dh)) !== false) {
            if (substr($file, 0, 6) == "fiche_") {
                if ($fiche = simplexml_load_file($dir . "/" . $file, null, LIBXML_DTDVALID)) {
                    ...
                    $dataset->dats_title = makeDatsTitle(
                        $fiche->xpath('id'),
                        $fiche->xpath('description/name'),
                    ...
                    $dataset->dats_sensors = makeDatsSensor(
                        $fiche->xpath('description/name'),
                        $fiche->xpath('description/trademark'),
                        $fiche->xpath('description/model'),
                    ...
                    $dataset->boundings = makeDatsBoundings(
                        $fiche->xpath('geographicalcoverage/geographicalform/point'),
