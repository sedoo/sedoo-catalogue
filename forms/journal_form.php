...
class journal_form extends login_form {
  ...
  function displayAddForm($typeJournal = 0) {
    ...
    echo '<table>';
    echo '<tr><td><strong>' . $this->getElement('type_journal')->getLabel() . '</strong></td><td>' . $this->getElement('type_journal')->toHTML() . '</td></tr>';
    echo '<tr><td><strong>' . $this->getElement('contact')->getLabel() . '</strong></td><td>' . $this->getElement('contact')->toHTML() . '</td></tr>';
    if ($typeJournal != TYPE_CHANGES) {
