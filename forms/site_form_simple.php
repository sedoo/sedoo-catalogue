...
class site_form_simple extends base_form
{
    ...
    function saveForm()
    {
      ...
      // SITE
        ...
        } elseif (empty($dataset->sites[0]->place_id)) {
    ...
    function displayForm()
    {
      ...
      // Affichage des erreurs
        ...
        } elseif (strpos($_SERVER['REQUEST_URI'], '?datsId')) {
            $reqUri = substr($_SERVER['REQUEST_URI'], 0, strpos($_SERVER['REQUEST_URI'], '?datsId'));
        } else {
       ...
       */
        for ($i = 0; $i < $this->dataset->nbSensors; $i++) {
            ...
            echo '<strong>Instrument ' . ($i + 1) . '</strong></th></tr>';
          ...
          // Variables
            for ($j = 0; $j < $this->dataset->dats_sensors[$i]->nbVars; $j++) {
                ...
                echo '<strong>Parameter ' . ($j + 1) . ' measured by instrument ' . ($i + 1) . '</strong>' . $this->getElement('var_id_' . $i . '_' . $j)->toHTML() . '</td></tr>';
