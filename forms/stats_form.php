...
class stats_form extends login_form
{
    ...
    public function getNbEnregistrements()
    {
        ...
        } elseif ($this->projectName != MainProject) {
            ...
            if ($ldapbind) {
                ...
                while ($entry) {
                  ...
                  //$dn = ldap_get_dn($ldapconn, $entry);
                    ...
                    if (!array_key_exists($year, $resultat)) {
                        $resultat[$year][0] = 0;
                    }

                    if (!array_key_exists($month, $resultat[$year])) {
                        $resultat[$year][$month] = 0;
                    }

                    if (!array_key_exists($c, $resultat['c'])) {
                        $resultat['c'][$c] = 0;
                    }
    ...
    public function getNbRequetesByMonth()
    {
        ...
        if ($resultat = $bd->get_data($query)) {
            for ($i = 0, $size = count($resultat); $i < $size; $i++) {
                ...
                if (!array_key_exists($year, $requetes)) {
                    $requetes[$year][0] = 0;
                }
