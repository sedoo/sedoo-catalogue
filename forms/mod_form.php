...
class mod_form extends base_form{
	...
	function createForm(){
		...
		$k_select = $k->chargeFormPlat($this, 'model_categ'.$i, 'Model type');
	...
	function initForm(){
		...
		if (isset($this->dataset->sites) && !empty($this->dataset->sites)){
			...
			if ( isset($this->dataset->sites[0]) && !empty($this->dataset->sites[0]) ){
				$this->getElement('model')->setSelected($this->dataset->sites[0]->place_id);
				$this->getElement('new_model')->setValue($this->dataset->sites[0]->place_name);

			if (isset($this->dataset->sites[0]->gcmd_plateform_keyword) && !empty($this->dataset->sites[0]->gcmd_plateform_keyword)){
					$table = array();
					$gcmd = $this->dataset->sites[0]->gcmd_plateform_keyword;
					...
					ksort($table);
					$this->getElement('model_categ')->setValue($table);
				}
	...
	function saveForm(){
		...
		$sitesPlat = $this->exportValue('model_categ');
		$pred_plat_id = 0;
		for ($j = 3;$j >= 0;$j--){
		...
		$this->dataset->sites[1]->gcmd_plat_id = $pred_plat_id;
		if ($this->dataset->sites[1]->gcmd_plat_id != 0)	{
			$this->dataset->sites[1]->gcmd_plateform_keyword = new gcmd_plateform_keyword;
			$this->dataset->sites[1]->gcmd_plateform_keyword = $this->dataset->sites[1]->gcmd_plateform_keyword->getById($dataset->sites[1]->gcmd_plat_id);
		}
	...
	function displayForm(){
		...
		echo '<tr><th colspan="4" align="center"><a name="a_param" ></a><b>'.TXT_measuredParameter.'</b></td></tr>';
				
		for ($i = 0; $i < $this->dataset->nbVars; $i++){
			echo '<tr><td colspan="4" align="center"><b>'.TXT_measuredParameter.' '.($i+1).'</b>'.$this->getElement('var_id_'.$i)->toHTML().'</td></tr>';
