...
class map_form extends login_form
{
    ...
    function createFormMap(
        ...
        $defaultLonMax = MAP_DEFAULT_LON_MAX
    ) {
        ...
        if ($_POST['minLat']) {
            $this->getElement('startMinLat')->setValue($_POST['minLat']);
        }

        if ($_POST['maxLat']) {
            $this->getElement('startMaxLat')->setValue($_POST['maxLat']);
        }

        if ($_POST['minLon']) {
            $this->getElement('startMinLon')->setValue($_POST['minLon']);
        }

        if ($_POST['maxLon']) {
            $this->getElement('startMaxLon')->setValue($_POST['maxLon']);
        }
    ...
    function deg2Double($deg, $min, $sec)
    {
        ...
        if ($d != 0) {
            $sign = $d / abs($d);
        } elseif ($deg . length > 1 && $deg . substring(0, 1) == "-") {
            $sign = -1;
        } else {
            $sign = 1;
        }
