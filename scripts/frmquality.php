...
if ($datsId > 0) {
  ...
  datstype($dats_type[0], $project_name); // in situ
  echo '</div>';
  echo "<div id='datsType=$dats_type[1]'>";
  datstype($dats_type[1], $project_name); // satellite
  echo '</div>';
  echo "<div id='datsType=$dats_type[2]'>";
  datstype($dats_type[2], $project_name); // model
  echo '</div>';
  echo '</div>';
  echo '</div>';
}
 ...
 * @param string $project_name ex: passy
 * @return void
 */
