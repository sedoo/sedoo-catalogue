...
class treeByPlat {
	...
	function isEmptyTypeTab() {
		...
		foreach ( $dats_type_ids as $type_id ) {
			...
			if ($sites_ids != NULL) $query_dp1 = "select distinct dats_id from dats_place where place_id in (select place_id from place where place_id in ( 
				$sites_ids ) or (pla_place_id in ( 
				$sites_ids ) and place_level is null))";
			else $query_dp1 = 0;
